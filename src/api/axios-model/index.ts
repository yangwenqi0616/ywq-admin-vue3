import { r, req, request, commonRequest } from '../http';
import type { R, Res, Response } from '../http';
import type { BaseParam, BaseResult } from '../types';
...
export const postDownloadFile = async (param: BaseParam): Promise<void> => {
  const { data, headers } = await request.post('/xx', param, {
    responseType: 'blob'
  });
...
export const getDownloadFile = async (param: any): Promise<void> => {
  const { data, headers } = await commonRequest.get('/XXX', param, {
    responseType: 'blob'
  });
  const name = decodeURI(headers['content-disposition'].split('=')[1]);
  downloadFile(name, data);
